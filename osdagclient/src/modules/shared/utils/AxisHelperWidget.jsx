...
const AxisHelperWidget = ({ orthographicView }) => {
  ...
  useEffect(() => {
    ...
    const xLabel = createTextSprite('X', '#ff0000', new THREE.Vector3(0.9, 0, 0));
    const yLabel = createTextSprite('Y', '#00ff00', new THREE.Vector3(0, 0.9, 0));
    const zLabel = createTextSprite('Z', '#0000ff', new THREE.Vector3(0, 0, -0.9));
  ...
  useFrame(() => {
    if (axisGroupRef.current && axisCameraRef.current && rendererRef.current && axisSceneRef.current) {
      if (orthographicView) {
